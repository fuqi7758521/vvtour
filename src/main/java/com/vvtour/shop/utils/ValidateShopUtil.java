 ...
 * 
 * */
public class ValidateShopUtil {

	private final static Logger logger = Logger.getLogger(ValidateShopUtil.class);
	
	public static<T extends BaseEntity> PublicResult<Boolean> checkRegistAdmin(ProductInfo info){
		PublicResult<Boolean> publicResult = new PublicResult<Boolean>(false);
		try{
			//==========================================验证用户名
			
			if(StringUtils.isBlank(info.getTour_title())){
