			...
			$(self).one("appear",function(){
				if(!this.loaded){
					$("<img />").bind("load",function(){
						$(self).hide().attr("src",$(self).attr("srcto"))[settings.effect](settings.effectspeed);
						self.loaded=true
					}).attr("src",$(self).attr("srcto"))
				}
			});
			if("scroll"!=settings.event){
...
(function($){
	var ui = UI;
	ui.extend.lazyload = function(opt){
		var option = {
			selector : $(this),
			effect : "fadeIn"
		}
		...
		selector.lazyload({
			placeholder : ui.site+"img/grey.gif",  
			effect : effect
		});
	}
})(jQuery);